...
type Reconciliation interface {
	...
	ReconcileCheck(ctx context.Context) (toCreate []types.Object, toDestroy []types.Object, err error)
	...
	Status(ctx context.Context) (map[string][]uint16, error)
}
...
// Some resources need others to already exist, so creating all resources returned by ReconcileCheck
// and then calling ReconcileCheck again will not always result in "nothing to change".
func (r *reconciliation) ReconcileCheck(ctx context.Context) ([]types.Object, []types.Object, error) {
	_, done := getOrCreateStateRetriever(ctx, r)
	defer done()

	if err := r.retrieveState(withBorrowedStateRetriever(ctx)); err != nil {
...
// is done.
func (r *reconciliation) Reconcile(ctx context.Context) error {
	_, done := getOrCreateStateRetriever(ctx, r)
	defer done()
	...
	for !completed {
		toCreate, toDestroy, err := r.ReconcileCheck(withBorrowedStateRetriever(ctx))
		if err != nil {
...
func (r *reconciliation) Status(ctx context.Context) (map[string][]uint16, error) {
	if err := r.retrieveState(ctx); err != nil {
		return nil, err
	}
...
func (r *reconciliation) retrieveState(ctx context.Context) error {
	ret, done := getOrCreateStateRetriever(ctx, r)
	defer done()
	...
	var err error
	r.remoteStateSnapshot, err = ret.FilteredState(r.ctx, r.lb.Identifier)
	if err != nil {
		return err
	}
