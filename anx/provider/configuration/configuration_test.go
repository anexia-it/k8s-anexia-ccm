...
func TestNewProviderConfig_nil(t *testing.T) {
	...
	if err := os.Unsetenv("ANEXIA_TOKEN"); err != nil {
		t.Fatalf("unsetenv failed: %v", err)
	}
	if err := os.Unsetenv("ANEXIA_CLUSTER_NAME"); err != nil {
		t.Fatalf("unsetenv failed: %v", err)
	}
...
func TestNewProviderConfig_env_overrides(t *testing.T) {
	if err := os.Setenv("ANEXIA_TOKEN", "bar"); err != nil {
		t.Fatalf("setenv failed: %v", err)
	}
	if err := os.Setenv("ANEXIA_CLUSTER_NAME", "envcluster"); err != nil {
		t.Fatalf("setenv failed: %v", err)
	}
	defer func() {
		_ = os.Unsetenv("ANEXIA_TOKEN")
	}()
	defer func() {
		_ = os.Unsetenv("ANEXIA_CLUSTER_NAME")
	}()
