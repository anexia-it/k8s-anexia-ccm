...
func (r *stateRetrieverImpl) storePublicAddress(lbID, addr string) {
	if idx := sort.SearchStrings(r.state[lbID].publicAddresses, addr); idx >= len(r.state[lbID].publicAddresses) || r.state[lbID].publicAddresses[idx] != addr {
		r.state[lbID].publicAddresses = append(r.state[lbID].publicAddresses, addr)
		sort.Strings(r.state[lbID].publicAddresses)
	}
}
...
func (r *stateRetrieverImpl) filterBinds(lbID string, allBinds []*lbaasv1.Bind) ([]*lbaasv1.Bind, error) {
	...
	for _, bind := range allBinds {
		idx, err := compare.Search(lbaasv1.Frontend{Identifier: bind.Frontend.Identifier}, r.state[lbID].frontends, "Identifier")
		if err != nil {
